...
import { NEWS_CATEGORY_LABELS } from "../news/parse";
import { NEWS_VISUAL, type ArticleCategory, type ArticleSummary } from "./meta";

// Presentational article pieces (no hooks): /haberler list and the article page.
...
function ArticleMeta({ article, className }: { article: ArticleSummary; className?: string }) {
  return (
    <span className={cn("flex items-center gap-1.5 text-xs text-muted-foreground", className)}>
      <span className="font-semibold text-foreground/80">{NEWS_CATEGORY_LABELS[article.category]}</span>
...
export function ArticleCard({ article }: { article: ArticleSummary }) {
  return (
    ...
    >
      <ArticleCover category={article.category} coverUrl={article.coverUrl} alt="" eager className="aspect-[16/9] w-full" />
      <div className="p-4">
...
export function ArticleRow({ article }: { article: ArticleSummary }) {
  return (
    ...
    >
      <ArticleCover category={article.category} coverUrl={article.coverUrl} alt="" className="size-20 shrink-0 rounded-2xl" iconClassName="size-8" />
