...
import { useDefaultDistrict } from "@/lib/location/store";
...
/** /duyurular list: kind chips, "İlçem" (saved district) filter, expiry re-checked on the device clock. */
export function AnnouncementsList({ items, renderedAt }: { items: Announcement[]; renderedAt: number }) {
  const now = useNow(renderedAt);
  const myDistrict = useDefaultDistrict();
  const [filter, setFilter] = React.useState<Filter>("all");

  const active = React.useMemo(() => sortAnnouncements(items.filter((a) => announcementPhase(a, now) !== "ended"), now), [items, now]);

  const isMine = React.useCallback((a: Announcement) => !!myDistrict && a.districts.some((d) => d.id === myDistrict), [myDistrict]);

  const options = React.useMemo<ChipOption<Filter>[]>(() => {
    const opts: ChipOption<Filter>[] = [{ value: "all", label: "Tümü", count: active.length }];
    const mineCount = active.filter((a) => isMine(a) || a.districts.length === 0).length;
    if (myDistrict && active.some(isMine)) opts.push({ value: "mine", label: "İlçem", count: mineCount });
  ...
  const visible =
    effective === "all"
      ? active
      : effective === "mine"
        ? active.filter((a) => isMine(a) || a.districts.length === 0)
