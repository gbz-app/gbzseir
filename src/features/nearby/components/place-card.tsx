...
import { placeCategoryMeta } from "../config";
...
export function PlaceCard({ place, priority }: { place: PlaceSummary; priority?: boolean }) {
  const meta = placeCategoryMeta(place.details.category);
  return (
    <Link
    ...
    >
      <div className="relative">
        <PlaceVisual category={place.details.category} photo={place.details.photos[0]} name={place.name} priority={priority} className="aspect-[16/10]" />
...
export function PlaceRow({ place }: { place: PlaceSummary }) {
  ...
  return (
    ...
    >
      <PlaceVisual category={place.details.category} photo={place.details.photos[0]} name={place.name} sizes="48px" className="size-12 shrink-0 rounded-xl" iconClassName="size-6" />
