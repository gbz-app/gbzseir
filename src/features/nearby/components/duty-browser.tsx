...
import { Clock3, Loader2, LocateFixed, Navigation } from "lucide-react";
...
/**
 * D2 list: "Bugün / Yarın" tabs, filtered at render time (the HTML may come from ISR or the service worker), every
 * district of Kocaeli (no district picking, owner 12.09), sorted by distance when a GPS fix (or a district saved on this
 * device earlier) is known, refreshed automatically at the 08:30 switch.
 */
export function DutyBrowser({ rows, serverNow, fetchedAt, ok, mode }: DutyBrowserProps) {
  ...
  // Sort point: the GPS fix, else the centre of a district saved earlier.
  const refDistrict = districtBySlug(loc.district);
