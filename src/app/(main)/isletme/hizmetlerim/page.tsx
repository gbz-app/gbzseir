...
import { DISTRICT_SLUGS } from "@/config/districts";
...
/**
 * Where incoming requests come from: matching uses the firm's service categories and districts (edited on
...
async function RequestSourceNote({ business }: { business: OwnerBusiness }) {
  ...
  const areas = business.service_district_ids.length;
  const allDistricts = DISTRICT_SLUGS.every((slug) => business.service_district_ids.includes(slug));
    ...
    <section className="mb-4 rounded-2xl bg-card p-4" aria-labelledby="talep-kaynagi">
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-sm leading-relaxed text-muted-foreground">
            {categories.length
              ? "Müşteri talepleri, seçtiğin hizmet kategorilerine ve ilçelere göre gelir."
      ...
      {categories.length || areas ? (
        <ul className="mt-3 flex flex-wrap gap-1.5" aria-label="Talep ayarların">
          ...
          {areas ? (
            <li className="rounded-full bg-muted px-2.5 py-1 text-xs font-semibold text-muted-foreground">{allDistricts ? "Tüm Kocaeli" : `${areas} ilçe`}</li>
          ) : null}
        </ul>
      ) : null}
