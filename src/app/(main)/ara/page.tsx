...
import { Search, SearchX, Store, Tag, Wrench } from "lucide-react";
...
/** J1 - Genel arama (rpc global_search): hizmetler, işletmeler, ilanlar, yerler. */
export default async function SearchPage({ searchParams }: Props) {
  ...
  if (q.length >= 2) {
    ...
    else res = data as unknown as SearchResult;
  }
  const total = res ? res.listings.length + res.businesses.length + res.services.length + res.pois.length : 0;
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-6 px-4 pt-4 pb-8">
        ...
        {!q ? (
          <EmptyState icon={Search} title="Ne arıyorsun?" description="Eczane, cami, usta, ilan, işletme ya da gezilecek yer ara." />
        ) : q.length < 2 ? (
